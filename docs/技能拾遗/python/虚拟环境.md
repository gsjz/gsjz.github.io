...
=== "windows"
    ...
    推荐的做法是使用新安装出来的 `Anaconda Prompt`。
    
    （对比来说，我们熟悉的 `cmd` 其实全称是 `Command Prompt` 命令提示符。只不过 `prompt` 在如今的 AI 泡沫中人们逐渐熟知另一个指代了。）
...
=== "linux"
    ...
    下好后还不能直接运行，需要使用 `chmod +x name.sh` 来给 `sh` 加权限。

    通过 `.sh` 文件安装的 `conda` 需要手动找到 `activate` 文件然后 `source`，例如 `source ~/anaconda3/bin/activate`。
...
???+ info "conda init"
    ...
    然而我个人感觉每次另外打开一个 anaconda prompt 还是太麻烦了。
    
    最开始安装我也没有选修改 PATH，查了一下应该怎么办。

    后面发现建议说在 anaconda prompt 里面运行一遍

    ```
    conda init
    ```

    这样系统自带的 powershell 就能用了。据说这样和自己改 PATH 还不一样

    | 对比项                         | conda init | 手动 PATH |
    | --------------------------- | ---------- | ------- |
    | conda 命令可用                  | ✅          | ✅       |
    ...
    我体感理解上的差别就是，这样默认打开终端后都会默认进入 conda 的 base 环境。

    和系统环境等价？但并不想用 conda，打开的时候也要默认加载，会慢不少？
    ...
    这样还有一个神秘的后果是，在系统中直接打开 powershell 会出来一段红字，并且无法使用 conda。
    
    （对比之下，通过 vscode 打开的 powershell 终端则不会。）

    这主要是系统的防御机制导致的，希望不要 conda 自动运行脚本。可以通过
...
???+ warning "假如不固定 python 版本"
    如果在新建 conda 环境的时候不指定 python=3.x 等，可能会导致无法直接使用 python 而只能使用 python3
